...
class OCRProcessor:
    ...
    async def process_attachment(self, attachment):
        ...
        :return: Texte extrait de l'image ou message d'erreur.
        """
        try:
            # Télécharger l'image localement
            local_filename = os.path.join(settings.UPLOAD_IMAGES, attachment.filename)
            ...
            return extracted_text if extracted_text else "❌ Aucun texte détecté dans l'image."
        except Exception as e:
